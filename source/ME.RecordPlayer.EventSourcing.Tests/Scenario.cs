...
namespace ME.RecordPlayer.EventSourcing.Tests
{
  public class Scenario : INotifyCompletion
  {
    ...
    public static Scenario Given(Action precondition) => new Scenario().Then(precondition);

    public static Scenario Given(Func<ValueTask> preconditionTask) => new Scenario().Then(() => preconditionTask().AsTask());
